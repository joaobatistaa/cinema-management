    ...
    try {
      ...
      try {
        await addAuditLog({
          ...
          description: `Nova transação de bar registrada: ${
            cart.length
          } itens, total ${transaction.total.toFixed(2)}€`,
      ...
      if (email) {
        let details = `
          ...
          <p><strong>Data:</strong> ${new Date(transaction.date).toLocaleString(
            "pt-PT"
          )}</p>
          <p><strong>Descrição:</strong> ${transaction.desc || ""}</p>
          <p><strong>NIF:</strong> ${
            transaction.nif ? transaction.nif : "Consumidor final"
          }</p>
          <h3>Produtos:</h3>
          <ul>
            ${transaction.items
          ...
          <p><strong>Total:</strong> ${Number(transaction.total).toFixed(
            2
          )}€</p>
        `;
        try {
          await sendEmail({
            ...
            text: `Detalhes da sua transação:\nData: ${new Date(
              ...
              .map(
                (item) =>
                  `${item.name} - ${item.quantity} x ${Number(
                    item.price
                  ).toFixed(2)}€ = ${(
                    Number(item.price) * Number(item.quantity)
                  ).toFixed(2)}€`
          ...
          console.error(
            "Erro ao enviar email de confirmação de transação:",
            err
          );
        }
      }
